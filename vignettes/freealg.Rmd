...
The package can substitution with the `subs()` function:
...
Starting with a univariate example we have:
...
One might expect that $d(X^3)/da=3X^2dX/da$, but:

```{r}
deriv(X^3,1) == 3*X^2*deriv(X,1)
```

Now we check Young's theorem, that partial derivatives commute:
