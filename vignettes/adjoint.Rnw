...
An associative algebra ${\mathcal A}$ and $X,Y\in{\mathcal A}$, we
define the \emph{Lie Bracket} $[X,Y]$ as $XY-YX$.  In the {\tt
...
\subsection*{The adjoint: definition}
...
homomorphism.  If $\phi$ is a Lie homomorphism then $\phi([x,y]) =
